...
var senRestorePassword = function (user, newPassword) {
  var message = {
    subject: 'Restore password for eventorio',
    from_email: 'support@eventorio.uk.to',
    from_name: 'Support@Eventorio',
    to: [{
      email: user.email,
      name: user.username,
      type: 'to'
    ...
    html: [
    '<p>Hi, <b>' + user.username + '</b>. You asked us to restore your password.<p>',
    '<p>Here is the new one: <b>' + newPassword + '</b></p>',
    '<p>Good luck!',
    ].join('')
  };
  ...
  app.email.messages.send({
    ...
    return app.logger.info('Restore password message sent to ' + user.email);
  }, function (err) {
    err.message = 'Mandrill error, Restore password message: ' + err.name + ' - ' + err.message;
    return app.err(err);
  });
}